...
# 2. Install dependencies (only numpy; the engine itself is stdlib-only)
pip install -r requirements.txt

# 3. Provide a Gemini API key for image OCR (used only to extract the
#    amounts of blank-amount events referenced by dataset/images.csv)
#    - create .env in the repo root (it is gitignored, never commit it):
#        GEMINI_API_KEY=<paste your key here>
#    - optionally override the model with GEMINI_MODEL=gemini-2.5-flash

# 4. OPTIONAL: (re-)extract image amounts with Gemini OCR. The final run
#    does NOT need this step or any API key - the 16 extracted amounts are
#    already cached in code/perception/__init__.py.
python3 code/ocr_gemini.py            # all 16 images
python3 code/ocr_gemini.py image_14   # a single image

# 5. Run the test suite (forecast calibration, policy ranking, contract)
python3 -m unittest discover code/tests

# 6. Final run (writes output.csv for all 250 requests in dataset/requests.csv,
#    plus code/emit/run_manifest.json for reproducibility)
python3 code/main.py
```

Notes:

- No credentials are needed for anything except the Gemini OCR step; the
  Google Generative Language API key is read from `.env` (or the
  `GEMINI_API_KEY` environment variable) at runtime and is never logged.
- Everything runs offline after setup: no banking, market-data, or
  exchange-rate calls are made.
